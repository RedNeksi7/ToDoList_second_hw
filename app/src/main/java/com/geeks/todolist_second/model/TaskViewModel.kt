package com.geeks.todolist_second.model

import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import androidx.lifecycle.map
import com.geeks.todolist_second.repository.TaskRepository

class TaskViewModel : ViewModel() {
    private val repository = TaskRepository()

    private val _filterType = MutableLiveData(Filter.ALL)
    val filterType: LiveData<Filter>
        get() = _filterType

    private val _tasks = MutableLiveData<List<Task>>()
    val tasks: LiveData<List<Task>>
        get() = _tasks

    val filteredTasks: LiveData<List<Task>> = _tasks.map { tasks ->
        when (_filterType.value) {
            Filter.COMPLETED -> tasks.filter { it.isCompleted }
            Filter.INCOMPLETE -> tasks.filter { !it.isCompleted }
            else -> tasks
        }
    }

    init {
        _tasks.value = repository.getAllTasks()
    }

    fun addTask(title: String) {
        repository.addTask(title)
        _tasks.value = repository.getAllTasks()
    }

    fun updateTask(task: Task) {
        repository.updateTask(task)
        _tasks.value = repository.getAllTasks()
    }

    fun deleteTask(task: Task) {
        repository.deleteTask(task)
        _tasks.value = repository.getAllTasks()
    }

    fun setFilter(filterType: Filter) {
        _filterType.postValue(filterType)
    }

    fun editTask(task: Task) {
        repository.updateTask(task)
        _tasks.value = repository.getAllTasks()
    }

}